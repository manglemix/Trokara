...
export var interpolation_weight := 6.0				# If interpolation is not desired, set to 1
...
func get_new_basis(delta: float, current_basis: Basis) -> Basis:
	var rotation := basis_node.global_transform.basis.get_euler()
	
	if not enable_pitch_rotation:
		rotation.x = 0
	
	return current_basis.slerp(Basis(rotation), interpolation_weight * delta)
